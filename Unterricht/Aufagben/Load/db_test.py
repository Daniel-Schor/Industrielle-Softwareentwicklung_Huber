...
class DatabaseConnectionTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.conn = sqlite3.connect(
            r"Unterricht\Aufagben\Load\FinancialSample.db")
    ...
    def test_database_connection(self):
        ...
        self.assertIsNotNone(
            self.conn, "Datenbankverbindung konnte nicht hergestellt werden.")
...
class RowCountTest(unittest.TestCase):
    ...
    def test_number_of_rows(self):
        ...
        self.assertEqual(count, 700, f"Erwartete 700 Zeilen, aber {
                         count} gefunden.")
    ...
    def test_number_of_rows_falsch(self):
        ...
        self.cursor.execute("SELECT COUNT(*) FROM FinancialSample")
        count = self.cursor.fetchone()[0]
        self.assertFalse(count != 700, f"Erwartete 700 Zeilen, aber {
                         count} gefunden.")
...
class SalesValueTest(unittest.TestCase):
    ...
    def test_max_sales(self):
        ...
        cleaned_sales = [locale.atof(
            sales[0].strip().replace('$', '')) for sales in sales_data]
        max_sales = max(cleaned_sales)
        self.assertEqual(max_sales, 1159200, f"Erwartete 1159200, aber {
                         max_sales} gefunden.")
    ...
    def test_max_sales_false(self):
        ...
        cleaned_sales = [locale.atof(
            sales[0].strip().replace('$', '')) for sales in sales_data]
        max_sales = max(cleaned_sales)
        self.assertFalse(max_sales != 1159200, f"Erwartete 1159200, aber {
                         max_sales} gefunden.")
...
class RandomDataCheckTest(unittest.TestCase):
    ...
    def test_random_data_check(self):
        ...
        self.assertGreater(
            len(rows), 0, "Keine Daten in der Tabelle gefunden.")

        sample_rows = random.sample(rows, min(5, len(rows)))
        for row in sample_rows:
            self.assertIsInstance(
                row[0], str, "Erwartete eine Zeichenkette in der ersten Spalte.")
            self.assertIsInstance(
                row[1], str, "Erwartete eine Zeichenkette in der zweiten Spalte.")
...
class ColumnIntegrityTest(unittest.TestCase):
    ...
    def test_columns_exist(self):
        ...
        for col in required_columns:
            self.assertIn(col, columns, f"Spalte '{
                          col}' fehlt in der Datenbank.")
